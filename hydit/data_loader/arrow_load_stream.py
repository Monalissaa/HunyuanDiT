...
class TextImageArrowStreamDBLoRA(Dataset):
    def __init__(self,
                 args,
                 resolution=512,
                 random_flip=None,
        ...
        self.batch_size = batch_size
        self.world_size = world_size
        self.index_manager, self.class_index_manager = self.load_index()
    ...
    def load_index(self):
        multireso = self.multireso
        index_file = self.index_file
        ## add the class index file
        class_index_file = self.class_index_file
        ...
        if multireso:
            ...
            if len(index_file) == 1:
                index_manager = ArrowIndexV2(index_file[0])
                self.log_fn(f"Using ArrowIndexV2: {len(index_manager):,}")
                ## add the class index manager
                class_index_manager = ArrowIndexV2(class_index_file[0])
                self.log_fn(f"Using ArrowIndexV2: {len(class_index_manager):,}")
            else:
                index_manager = MultiIndexV2(index_file)
                self.log_fn(f"Using MultiIndexV2: {len(index_manager):,}")

        return index_manager, class_index_manager

    def shuffle(self, seed, fast=False):
        self.index_manager.shuffle(seed, fast=fast)
        self.class_index_manager.shuffle(seed, fast=fast)

    def get_raw_image(self, index, image_key="image"):
        try:
            ret = self.index_manager.get_image(index, image_key)
            class_ret = self.class_index_manager.get_image(index, image_key)
        except Exception as e:
            self.log_fn(f'get_raw_image | Error: {e}')
            ret = Image.new("RGB", (256, 256), (255, 255, 255))
        return ret, class_ret
    ...
    def get_image_with_hwxy(self, index, image_key="image"):
        image, class_image = self.get_raw_image(index, image_key=image_key)
        origin_size = image.size
        ...
        if self.multireso:
            ...
            image_crop, crops_coords_top_left = self.random_crop_image(image, origin_size, target_size)
            image_tensor = self.flip_norm(image_crop)
            class_image_tensor = self.flip_norm(class_image)
        ...
        return image_tensor, class_image_tensor, kwargs
    ...
    def get_original_text(self, ind):
        text = self.index_manager.get_attribute(ind, 'text_zh' if self.enable_CN else 'text_en')
        class_text = self.class_index_manager.get_attribute(ind, 'text_zh' if self.enable_CN else 'text_en')
        text = str(text).strip()
        class_text = str(class_text).strip()
        return text, class_text

    def get_text(self, ind):
        text, class_text =  self.get_original_text(ind)
        if text == '':
            text = '随机生成一张图片'
        return text, class_text
    ...
    def __getitem__(self, ind):
        ...
        if random.random() < self.uncond_p:
            ...
            description, class_description = self.get_text(ind)
        
        # Get text for t5
        if random.random() < self.uncond_p_t5:
            description_t5 = ""
        else:
            description_t5, class_description_t5 = self.get_text(ind)

        original_pil_image, class_original_pil_image, kwargs = self.get_image_with_hwxy(ind)

        # Use encoder to embed tokens online
        text, text_embedding, text_embedding_mask = self.get_text_info_with_encoder(description)
        class_text, class_text_embedding, class_text_embedding_mask = self.get_text_info_with_encoder(class_description)

        text_t5, text_embedding_t5, text_embedding_mask_t5 = self.get_text_info_with_encoder_t5(description_t5)
        class_text_t5, class_text_embedding_t5, class_text_embedding_mask_t5 = self.get_text_info_with_encoder_t5(class_description_t5)
        return (
            original_pil_image,
            class_original_pil_image,
            text_embedding.clone().detach(),
            class_text_embedding.clone().detach(),
            text_embedding_mask.clone().detach(),
            class_text_embedding_mask.clone().detach(),
            text_embedding_t5.clone().detach(),
            class_text_embedding_t5.clone().detach(),
            text_embedding_mask_t5.clone().detach(),
            class_text_embedding_mask_t5.clone().detach(),
            {k: torch.tensor(np.array(v)).clone().detach() for k, v in kwargs.items()},
        )
